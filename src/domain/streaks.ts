...
import { countCompletions, isDone } from './completion';
...
function scheduledDayStatuses(habit: Habit, completions: Completion[], from: Date, to: Date): boolean[] {
  if (from > to) return [];
  return eachDayOfInterval({ start: from, end: to })
    .filter((d) => isScheduledOn(habit.daysOfWeek, d))
    .map((d) => isDone(countCompletions(completions, habit.id, toLocalDateString(d)), habit.targetPerDay));
}

export function currentStreak(habit: Habit, completions: Completion[], now: Date = new Date()): number {
  const statuses = scheduledDayStatuses(habit, completions, habitCreatedLocalDate(habit), now);
...
export function bestStreak(habit: Habit, completions: Completion[], now: Date = new Date()): number {
  const statuses = scheduledDayStatuses(habit, completions, habitCreatedLocalDate(habit), now);
  let best = 0;
  let running = 0;
...
export function consistency30d(habit: Habit, completions: Completion[], now: Date = new Date()): number | null {
  const windowStart = new Date(now);
  windowStart.setDate(windowStart.getDate() - 29);
  const createdLocal = habitCreatedLocalDate(habit);
  const from = windowStart > createdLocal ? windowStart : createdLocal;

  const statuses = scheduledDayStatuses(habit, completions, from, now);
